...
partial class AppSettings : ISettingsRoot
{
  ...
  private PropertyBacker<string[]?>? _recentFilesBacker;
  public partial string[]? RecentFiles
  {
    get => _recentFilesBacker?.Value ?? default;
    set
    {
      if (_recentFilesBacker!.TrySet(value))
      {
        OnPropertyChanged(new System.ComponentModel.PropertyChangedEventArgs(nameof(RecentFiles)));
        OnRecentFilesChanged(value);
      }
    }
  }

  partial void OnRecentFilesChanged(string[]? value);
  /// <inheritdoc/>
  ...
  {
    _recentFilesBacker ??= new PropertyBacker<string[]?>(keyPrefix, nameof(RecentFiles), default !);
    _recentFilesBacker.SetStore(store);
    if (_recentFilesBacker.GetInitialValue(default !))
    {
      OnPropertyChanged(new System.ComponentModel.PropertyChangedEventArgs(nameof(RecentFiles)));
      OnRecentFilesChanged(_recentFilesBacker.Value);
    }
  }
